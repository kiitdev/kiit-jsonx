 ...
 *
 * [ch] is the single literal character [JsonLexer] dispatches on for the six punctuation token
 * types (`{`, `}`, `[`, `]`, `:`, `,`) — null for every other entry. A [JNumber] can start with
 * `-` or any digit, and [JTrue]/[JFalse]/[JNull] are multi-character keywords whose first letter
 * is only a dispatch hint, not the token's identity — modeling either of those as `ch` would be
 * misleading, so they're deliberately left null rather than forced to fit.
 */
